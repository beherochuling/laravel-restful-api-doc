...
class DocMarker {
    public static function getUri($class,$method) {   //echo $class,$method;echo "\n";
        $all = Route::getRoutes();
        ...
        foreach ($all as $route) {
            ...
            if ($action[0] == $class && $action[1] == $method)
            {
                ...
                $md = $methods[0];
                if (strstr($uri,'processesxx')!==false ) continue;
        ...
        if (count($result) == 0) die($class.' '.$method.' 找不到对应路由');
    ...
    public static function getEnumDoc($path = '/app/Enum',$base_class='\App\Http\Enum') {
        ...
        foreach ($classes as $class) {
            if ( !$doc = DocParser::get_classes_commentdoc($class) ) continue;
            if ( !is_subclass_of($class,$base_class) ) continue;
            if ( isset($doc['ignore']) ) continue;
            if ( !isset($doc['intro']) ) die($class."缺少intro");
    ...
    public static function getDoc($path = '/app/Http/Controllers/Api',$base_class = '\App\Http\Controllers\Api') {
        ...
        foreach ($classes as $class) {
            if (!is_subclass_of($class,$base_class)) continue;
            if (!$doc = DocParser::get_classes_commentdoc($class,\Exception::class)) continue;
            if (!isset($doc['module'])) die($class."缺少module");
            if (isset($doc['ignore'])) continue;
            
            $module = $doc['module'];
            if (!isset($docs[$module])) $docs[$module] = [];
    ...
    public static function getExceptionDoc($path= '/app/Exceptions',$base_class='\App\Http\Exception') {
        ...
        foreach ($classes as $class) {
            if (substr($class, -9) !== 'Exception') continue;
            if (!is_subclass_of($class,$base_class)) continue;
            if (!$doc = DocParser::get_classes_commentdoc($class,\Exception::class)) continue;
            
            if (substr($doc['name'], 0,4) !== 'App\\') continue;
            ...
            if (!isset($doc['errno']) ) die(" {$doc['name']}中的错误码 无法读取 errno \n");
            if (isset($errnoList[$doc['errno']]) && $doc['errno'] != 500000 ) die(" {$doc['name']}中的错误码和 {$errnoList[$doc['errno']]} 中的重复 {$doc['errno']} \n");
    ...
    public static  function include_classes($directories) {
        ...
        foreach ($directories as $directory) {
            ...
            foreach ( $it as $o ) {
                ...
                if ( !preg_match('/\.php$/', $o->getPathName()) ) continue;
